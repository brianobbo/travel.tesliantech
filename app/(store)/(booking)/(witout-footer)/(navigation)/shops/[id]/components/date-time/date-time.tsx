...
export const BookingDateTime = ({
  ...
  const currentTimes = todayTimes?.times.filter((time) => dayjs(time, "HH:mm").isAfter());
  const disabledDays =
  ...
  const handleClickTimeSlot = (slot: string) => {
    dispatch({
      ...
      payload: { date: dayjs(startDate).toString(), time: slot },
    });
  };
  ...
  const renderTimeSlot = () => {
    const nextAvailableSlot = data?.data.find((item) =>
      item.times.some((time) => dayjs(time, "HH:mm").isAfter())
    );
